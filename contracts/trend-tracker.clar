...
(define-private (calculate-trend-score (views uint) (purchases uint) (engagement uint))
  (+
    (* views VIEW-WEIGHT)
    (* purchases PURCHASE-WEIGHT)
...
(define-private (update-trend-status (product-id uint) (trend-score uint))
  (let
    (
...
(define-public (register-product (product-id uint) (name (string-ascii 100)))
  (let
    (
  ...
  (let
    ...
    (update-trend-status 
      ...
      (calculate-trend-score 
        new-view-count 
        (get purchase-count product-data) 
